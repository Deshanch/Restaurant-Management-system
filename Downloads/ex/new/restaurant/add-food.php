...
<?php 
	...
	if (isset($_POST['submit'])) {//is admin has submit the form,this global variable is post as our method is post
		...
		$req_fields = array('food_no', 'item_name', 'price', 'availability');
		...
		$query = "SELECT * FROM foodbeverages WHERE F_NO = '{$food_no}' LIMIT 1";//$email is the sanitized variable
		...
		if (empty($errors)) {//if there is no error was found then we can send the data to the database 
			...
			$query = "INSERT INTO foodbeverages ( ";
			$query .= "F_NO,Item,Price,Availability";
			$query .= ") VALUES (";
			$query .= "'{$food_no}', '{$item_name}', {$price}, {$availability}";
			$query .= ")";
			...
			if ($result) {
				...
				header('Location: foods.php?food_added=true');
			} else {
...
<body>
	...
	<main>
		<h1>Add New Food<span> <a href="foods.php">< Back to Food List</a></span></h1><!-- back to user page -->
		...
		<form action="add-food.php" method="post" class="userform"><!-- creating the form -->
