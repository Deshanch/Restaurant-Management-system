...
<?php 
	...
	if (isset($_GET['deliveryboy_no'])) {//if the parameter passed to this page is set
		// getting the user information
		$emp_id = mysqli_real_escape_string($connection, $_GET['deliveryboy_no']);//sanitizing
		...
		if ($result_set) {//if result_set is set
			if (mysqli_num_rows($result_set) == 1) {//if there is a matchig row
				...
				// user not found
				header('Location:delivery-boys.php?err=not_found');	
			}
		} else {
			// query unsuccessful
			header('Location:delivery-boys.php?err=query_failed');
		}
	}
	...
	if (isset($_POST['submit'])) {
		...
		if (empty($errors)) {
			...
			if ($result) {
				...
				header('Location:delivery-boys.php?employee_modified=true');
			} else {
...
<head>
	<meta charset="UTF-8">
	<title>View / Modify Delivering Staff</title>
...
<body>
	...
	<main>
		<h1>View / Modify Delivering Staff<span> <a href="delivery-boys.php">< Back to List</a></span></h1>
		...
		<form action="modify-deliveryboys.php" method="post" class="userform">
