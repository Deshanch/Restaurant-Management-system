...
<?php 
	...
	if (isset($_POST['submit'])) {//is admin has submit the form,this global variable is post as our method is post
		...
		$query = "SELECT * FROM deliveryboy WHERE EmployeeId = '{$employee_id}' LIMIT 1";//$email is the sanitized variable
		...
		if (empty($errors)) {//if there is no error was found then we can send the data to the database 
			...
			$query1 = "INSERT INTO deliveryboy ( ";
			$query1 .= "EmployeeId";
			$query1 .= ") VALUES (";
			$query1 .= "'{$employee_id}'";
			$query1 .= ")";
			...
			if ($result && $result1) {
				...
				header('Location: delivery-boy.php?employee_added=true');
			} else {
...
<body>
	...
	<main>
		<h1>Add New Member<span> <a href="delivery-boys.php">< Back to List</a></span></h1><!-- back to user page -->
		...
		<form action="add-delivery-boys.php" method="post" class="userform"><!-- creating the form -->
