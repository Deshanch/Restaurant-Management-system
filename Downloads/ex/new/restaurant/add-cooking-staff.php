...
<?php 
	...
	if (isset($_POST['submit'])) {//is admin has submit the form,this global variable is post as our method is post
		...
		$query = "SELECT * FROM cookingstaff WHERE EmployeeId = '{$employee_id}' LIMIT 1";//$email is the sanitized variable
		...
		if ($employee_id[0] != 'C') {//if query is succedful
		...
		if (empty($errors)) {//if there is no error was found then we can send the data to the database 
			...
			$query1 = "INSERT INTO cookingstaff ( ";
			$query1 .= "EmployeeId";
			$query1 .= ") VALUES (";
			$query1 .= "'{$employee_id}'";
			$query1 .= ")";
			...
			if ($result && $result1) {
				...
				header('Location: cooking-staff.php?employee_added=true');
			} else {
...
<body>
	...
	<main>
		<h1>Add New Member<span> <a href="cooking-staff.php">< Back to List</a></span></h1><!-- back to user page -->
		...
		<form action="add-cooking-staff.php" method="post" class="userform"><!-- creating the form -->
