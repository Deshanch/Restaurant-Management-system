...
<?php 
	...
	if (isset($_POST['submit'])) {//is admin has submit the form,this global variable is post as our method is post
		...
		$req_fields = array('employee_id', 'bill_no', 'amount');
		...
		$query1 = "SELECT * FROM employee WHERE Employeeid = '{$employee_id}' LIMIT 1";//$email is the sanitized variable
		...
		if ($employee_id[0] != 'D') {//if query is succedful
		...
		if ($result_set1) {//if query is succedful
			if (mysqli_num_rows($result_set) == 0) {//if there is any row mean there is same email as entred
				$errors[] = 'This delivery boy not found';
			}
		}
		...
		if (empty($errors)) {//if there is no error was found then we can send the data to the database 
			...
			$query1 = "INSERT INTO delivering ( ";
			$query1 .= "Bill_NO, EMP_ID, Amount";
			$query1 .= ") VALUES (";
			$query1 .= "{$bill_no}, '{$employee_id}', {$amount}";
			$query1 .= ")";
			...
			if ($result && $result1) {
				...
				header('Location: delivery.php?employee_added=true');
			} else {
...
<body>
	...
	<main>
		<h1>Add New Member<span> <a href="delivery.php">< Back to List</a></span></h1><!-- back to user page -->
		...
		<form action="add-delivery.php" method="post" class="userform"><!-- creating the form -->
