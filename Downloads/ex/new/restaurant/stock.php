...
<?php 
	...
	$query = "SELECT * FROM rawmaterial GROUP BY rawmaterial.I_NO";
	$stocks = mysqli_query($connection, $query);//pass the query

	verify_query($stocks);

	while ($stock = mysqli_fetch_assoc($stocks)) {//fetch data row by row to associative array
		$stock_list .= "<tr>";
		$stock_list .= "<td>{$stock['I_NO']}</td>";
		$stock_list .= "<td>{$stock['ItemName']}</td>";
		$stock_list .= "<td>{$stock['avalibility']}</td>";
		//$stock_list .= "<td><a href=\"modify-stocks.php?item_no={$stock['I_NO']}\">Edit</a></td>";
		//user_id={$user['id']}--->this provides the related id for the page making global variable to access from any where using _GET
		$stock_list .= "<td><a href=\"delete-stocks.php?item_no={$stock['I_NO']}\"
...
<body>
	...
	<main>
		<h1>Stock &nbsp;&nbsp;<span><a href="add-stock.php">+ Add New</a></span></h1>
