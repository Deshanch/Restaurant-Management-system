...
<?php 
	...
	$query = "SELECT * FROM user WHERE is_deleted=0 ORDER BY first_name";//getting all the results
	$users = mysqli_query($connection, $query);//pass the query

	verify_query($users);

	while ($user = mysqli_fetch_assoc($users)) {//fetch data row by row to associative array
		$user_list .= "<tr>";
...
<body>
	<div class="wrapper">
		...
		<main>
			<h1>Management<span><a href="add-user.php">+ Add New</a></span></h1>
