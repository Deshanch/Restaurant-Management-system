...
<?php 
	...
	if (isset($_GET['waiter_no'])) {//if the parameter passed to this page is set
		// getting the user information
		$emp_id = mysqli_real_escape_string($connection, $_GET['waiter_no']);//sanitizing
		...
		if ($result_set) {//if result_set is set
			if (mysqli_num_rows($result_set) == 1) {//if there is a matchig row
				...
				// user not found
				header('Location: waiters.php?err=not_found');	
			}
		} else {
			// query unsuccessful
			header('Location: waiters.php?err=query_failed');
		}
	}
	...
	if (isset($_POST['submit'])) {
		...
		if (empty($errors)) {
			...
			if ($result) {
				...
				header('Location:waiters.php?employee_modified=true');
			} else {
...
<head>
	<meta charset="UTF-8">
	<title>View / Modify Waiter Staff</title>
...
<body>
	...
	<main>
		<h1>View / Modify Waiter Staff<span> <a href="waiters.php">< Back to List</a></span></h1>
		...
		<form action="modify-waiters.php" method="post" class="userform">
